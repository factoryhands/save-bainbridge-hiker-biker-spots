function sendEmail() {
  ...
  let mailtoLink = `mailto:tomb@biparks.org?subject=${encodeURIComponent(
    subject
  )}&body=${encodeURIComponent(emailBody)}`;

  if (name) {
    mailtoLink += `%0D%0A%0D%0A%0D%0AYours sincerely,%0D%0A${encodeURIComponent(
      name
    )}`;
  }
...
function typeWriter(elementId, txt, i) {
  if (i < txt.length) {
    ...
    setTimeout(function () {
      typeWriter(elementId, txt, i + 1);
    }, 10); // Reduced delay to speed up the typewriter effect
  }
}
